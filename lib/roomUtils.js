...
export function pickColor(room) {
  const used = new Set(Array.from(room.players.values()).map((p) => p.color).filter(Boolean));
  const paletteChoice = PALETTE.find((c) => !used.has(c));
  if (paletteChoice) return paletteChoice;
  // Fallback: generate a unique random color if palette is exhausted.
  let color = '';
  do {
...
export function computeResults(room) {
  ...
  // Assign area to the point on the right of each interval (area to the left of the point).
  const playerArea = new Map();
  for (let i = 0; i < allPoints.length; i++) {
    const current = allPoints[i];
    const leftBoundary = i === 0 ? 0 : allPoints[i - 1].x;
    const width = Math.max(0, current.x - leftBoundary);
    if (width <= 0) continue;
    playerArea.set(current.playerId, (playerArea.get(current.playerId) || 0) + width);
  }
